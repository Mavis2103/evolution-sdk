...
```typescript twoslash
import { Data, createClient, type UTxO } from "@evolution-sdk/evolution"
...
const tx = await client
  ...
  .collectFrom({
    inputs: orderUtxos,
    redeemer: {
      all: (inputs) => Data.constr(0n, [
        inputs.map(i => BigInt(i.index)) as Data.Data
      ]),
      inputs: orderUtxos
    }
  })
...
```typescript twoslash
import { Assets, Data, createClient } from "@evolution-sdk/evolution"
