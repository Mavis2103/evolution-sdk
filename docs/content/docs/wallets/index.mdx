...
**What wallets don't do**: Query blockchain, build transactions, or submit to network—those capabilities come from combining wallets with providers in [clients](/docs/clients).
...
|------|----------|----------|-------------|------------------|
| **Seed Phrase** | ✅ Local | ✅ | Memory/disk | Development, testing |
| **Private Key** | ✅ Local | ✅ | Secure vault | Backend automation |
| **API Wallet** | ❌ External | ✅ Via user | User's device | Frontend dApps |
| **Read-Only** | ❌ None | ❌ | N/A | Observation, monitoring |

## Usage with Clients

Wallets are configured when constructing clients, but the details of wiring a provider and submitting transactions belong in the clients documentation. A wallet page should only describe the wallet config shape and runtime responsibilities.

Example wallet configuration objects (pass these to your client factory—see the clients docs for wiring examples):

```typescript
const seedWalletConfig = { type: "seed", mnemonic: "...", accountIndex: 0 };
const privateKeyWalletConfig = { type: "private-key", paymentKey: "..." };
const apiWalletConfig = { type: "api", api: walletApi };
const readOnlyWalletConfig = { type: "read-only", address: "addr1..." };
```

See [Clients documentation](/docs/clients) for concrete examples that combine a provider and a wallet to build a client capable of querying and submitting transactions.
...
- **API Wallet**: Highest security, user controls all operations
- **Read-Only**: No security risk (no keys)

See [Security Best Practices](/docs/wallets/security) for detailed guidance.

## Next Steps

Explore each wallet type in detail:

- **[Seed Phrase Wallets](/docs/wallets/seed-phrase)** - Mnemonic-based signing
- **[Private Key Wallets](/docs/wallets/private-key)** - Direct key usage  
- **[API Wallets](/docs/wallets/api-wallet)** - CIP-30 integration
- **[Security](/docs/wallets/security)** - Best practices and checklists
- **[Clients](/docs/clients)** - Using wallets with providers