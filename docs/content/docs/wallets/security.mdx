...
const badConfig = {
  wallet: {
    type: "private-key",
    bech32PrivateKey: process.env.PRIVATE_KEY! // Bundled to client (DON'T DO THIS)
  }
};
```
...
const globalWalletConfig = { type: "seed", mnemonic: process.env.MNEMONIC! };
...
const walletConfig = { type: "seed", mnemonic: process.env.MNEMONIC };
```
...
async function loadMnemonicFromVault() {
  ...
  return { type: "seed", mnemonic } as const;
}
```
...
const badProviderConfig = { type: "blockfrost", projectId: "mainnetxxxxxxxxxxxx" };
...
const backendProviderConfig = { type: "blockfrost", projectId: process.env.BLOCKFROST_PROJECT_ID! };
const readOnlyWalletConfig = { type: "read-only", address: userAddress };
```
...
```typescript
// Account configs
const mainAccount = { type: "seed", mnemonic: process.env.MNEMONIC!, accountIndex: 0 };
const payoutAccount = { type: "seed", mnemonic: process.env.MNEMONIC!, accountIndex: 1 };
