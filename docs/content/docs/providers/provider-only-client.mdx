...
const utxosByRefs = await client.getUtxosByOutRef([
  new Core.TransactionInput.TransactionInput({ 
    transactionId: Core.TransactionHash.fromHex("abc123def456..."), 
    index: 0n 
  })
]);
...
// Datum resolution
const datum = await client.getDatum(new Core.DatumOption.DatumHash({ hash: Core.Bytes.fromHex("datum-hash-here") }));
...
const unsignedTx = Core.Transaction.fromCBORHex(unsignedTxCbor);
...
const signedTx = Core.Transaction.fromCBORHex(signedCbor);
const txHash = await client.submitTx(signedTx);
```
...
builder.payToAddress({
  address: Core.Address.fromBech32("addr1qxh7f8gxv43dxz7k2vf6x5cxj5f5mk5mmqfk5u7qp7t2nvw9pqp0aw50nln8nyzfh6fjp6sxgajx5q0c6p73xqf2qhvqwlx8f7"),
