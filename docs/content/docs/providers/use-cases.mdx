...
export async function trackDistribution(
  ...
  for (const address of addresses) {
    ...
    if (utxos.length > 0) {
      const amount = utxos.reduce((sum, utxo) => {
        return sum + (utxo.assets[tokenUnit] || 0n);
      }, 0n);
      
      totalSupply += amount;
      holders.push({ address, amount, percentage: 0 });
    }
  }
...
export async function trackPoolDelegators(
  ...
  return delegators.filter(
    (d) => d !== null && d.poolId === targetPoolId
  ) as Delegator[];
}
...
class ProviderWithFallback {
  ...
  async submitTx(txCbor: string) {
    return this.withFallback((client) => client.submitTx(txCbor));
  }
...
const utxos = await resilientProvider.getUtxos("addr1...");
```
