...
```typescript
import { Address, Assets, Transaction, createClient, cip30Wallet, mainnet } from "@evolution-sdk/evolution"
...
export async function buildTransaction(userAddress: string) {
  ...
  const client = createClient({
    ...
    wallet: readOnlyWallet(userAddress)
  })
...
export async function buildPayment(userAddressBech32: string, recipientAddressBech32: string, lovelace: bigint) {
  ...
  const client = providerClient.attachWallet(readOnlyWallet(userAddressBech32))
