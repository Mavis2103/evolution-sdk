...
describe("Transaction Tests", () => {
  let cluster: Devnet.DevNetCluster;
  ...
  beforeAll(async () => {
    ...
    genesisUtxos = await Devnet.Genesis.calculateUtxosFromConfigOrThrow(
      genesisConfig
    );
...
const client2 = createClient({
  ...
  const utxos1 = await Devnet.Genesis.calculateUtxosFromConfigOrThrow(genesisConfig);
