...
async function completeWorkflow() {
  ...
  const senderAddressHex = Core.Address.toHex(senderAddress);
  ...
  const receiverAddress = Core.Address.fromBech32(
  ...
  const signBuilder = await client
    ...
    .payToAddress({
      ...
      assets: Core.Assets.fromLovelace(10_000_000n) // Send 10 ADA
    })
...
const utxos = await providerClient.getUtxos(
  Core.Address.fromBech32("addr_test1qz2fxv2umyhttkxyxp8x0dlpdt3k6cwng5pxj3jhsydzer3n0d3vllmyqwsx5wktcd8cc3sq835lu7drv2xwl2wywfgs68faae")
);
...
describe("Transaction Tests", () => {
  ...
  beforeAll(async () => {
    ...
    const addressHex = Core.Address.toHex(await wallet.address());
  ...
  it("should submit simple payment", async () => {
    const signBuilder = await client
      .newTx()
      .payToAddress({
        address: Core.Address.fromBech32("addr_test1qz2fxv2umyhttkxyxp8x0dlpdt3k6cwng5pxj3jhsydzer3n0d3vllmyqwsx5wktcd8cc3sq835lu7drv2xwl2wywfgs68faae"),
  ...
  it("should handle multiple outputs", async () => {
    const signBuilder = await client
      ...
      .payToAddress({
        address: Core.Address.fromBech32("addr_test1qpq6xvp5y4fw0wfgxfqmn78qqagkpv4q7qpqyz8s8x3snp5n0d3vllmyqwsx5wktcd8cc3sq835lu7drv2xwl2wywfgsc3z7t3"),
...
async function multiWalletExample() {
  ...
  const addr1Hex = Core.Address.toHex(await wallet1.address());
  const addr2Hex = Core.Address.toHex(await wallet2.address());
...
const client2 = createClient({
  ...
  const signBuilder = await client1
    ...
    .payToAddress({
      ...
      assets: Core.Assets.fromLovelace(50_000_000n)
    })
    .build();
  ...
  const receivedUtxo = wallet2Utxos.find(
    u => Core.TransactionHash.toHex(u.transactionId) === Core.TransactionHash.toHex(txHash)
  );
