...
import { Address, Assets, TransactionHash, createClient, preprod, kupmios, seedWallet } from "@evolution-sdk/evolution"
...
async function completeWorkflow() {
  ...
  const wallet = createClient({
    chain: preprod,
    wallet: seedWallet({ mnemonic: MNEMONIC, accountIndex: 0 })
  })
  ...
  const client = createClient({
    chain: preprod,
    provider: kupmios({ kupoUrl: "http://localhost:1442", ogmiosUrl: "http://localhost:1337" }),
    wallet: seedWallet({ mnemonic: MNEMONIC, accountIndex: 0 })
  })
...
```typescript twoslash
import { Cluster, Config, Genesis } from "@evolution-sdk/devnet"
import { Address, Assets, TransactionHash, createClient, preprod, kupmios } from "@evolution-sdk/evolution"
...
describe("Transaction Tests", () => {
  ...
  beforeAll(async () => {
    ...
    const wallet = createClient({
      chain: preprod,
      wallet: seedWallet({ mnemonic, accountIndex: 0 })
    })
...
async function multiWalletExample() {
  ...
  const wallet1 = createClient({
    chain: preprod,
    wallet: seedWallet({ mnemonic: "wallet one mnemonic here", accountIndex: 0 })
  })

  const wallet2 = createClient({
    chain: preprod,
    wallet: seedWallet({ mnemonic: "wallet two mnemonic here", accountIndex: 0 })
  })
