...
assets = Core.Assets.addByHex(assets, policyId, assetName, 100n);
...
// ---cut---
const recipientAddress = Core.Address.fromBech32("addr_test1vrm9x2dgvdau8vckj4duc89m638t8djmluqw5pdrFollw8qd9k63");
...
const tx = await client
  ...
  .payToAddress({
    ...
    assets: Core.Assets.fromLovelace(paymentAmount)
  })
  .build();
```
...
const tx = await client
  ...
  .payToAddress({
    address: Core.Address.fromBech32("addr_test1vrm9x2dgvdau8vckj4duc89m638t8djmluqw5pdrFollw8qd9k63"),
    assets: Core.Assets.fromLovelace(amount)
  })
  .build();
```
